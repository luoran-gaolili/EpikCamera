...
public class SdofPhotoEntry extends FeatureEntryBase {
    ...
    private boolean hasVsdofMode(Activity activity) {
        if (CameraApiHelper.getLogicalCameraId() == null) {
            return false;
        }
        CameraCharacteristics cs = CameraUtil.getCameraCharacteristics(activity,
                CameraApiHelper.getLogicalCameraId());
        if (cs == null) {
            return false;
        }
        return CameraUtil.isSupportAvailableMode(cs,

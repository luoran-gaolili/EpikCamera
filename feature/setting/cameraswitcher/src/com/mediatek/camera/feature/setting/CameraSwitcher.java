...
public class CameraSwitcher extends SettingBase {
        ...
        switcher.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (SystemProperties.getInt(SWITCH_CAMERA_DEBUG_PROPERTY, 0) == 1) {
                    ...
                    //for Main2 debug
                    boolean debugMain2 = SystemProperties.getInt(DEBUG_MAIN2, 0) == 1;
                    if (debugMain2) {
                        newCameraId = mDataStore.getValue(KEY_DEBUG_STEREO_MAIN2,
                                "0", getStoreScope());
                        if (newCameraId.equals(BACK_CAMERA_ID)) {
                            newCameraId = FRONT_CAMERA_ID;
                        } else if (newCameraId.equals(FRONT_CAMERA_ID)) {
                            newCameraId = BACK_MAIN2_CAMERA_ID;
                        } else if (newCameraId.equals(BACK_MAIN2_CAMERA_ID)) {
                            newCameraId = BACK_CAMERA_ID;
                        }
                    }
                    LogHelper.i(TAG, "debug main2 cameraId = " + newCameraId
                            + ", debug switch: " + debugMain2);

                    boolean success = mApp.notifyCameraSelected(newCameraId);
                    if (success) {
                        LogHelper.d(TAG, "[onClick], switch camera success.");
                        mFacing = nextFacing;
                        mDataStore.setValue(KEY_DEBUG_STEREO_MAIN2, newCameraId,
                                getStoreScope(), true);
    ...
    private void switchCameraInDebugMode() {
        ...
        String nextFacing = requestCamera.substring(0, requestCamera.indexOf("-"));
        LogHelper.i(TAG, "[switchCameraInDebugMode] requestCamera " + requestCamera
                + ",resultCameraId " + resultCameraId + ",mFacing " + mFacing +
                ",nextFacing " + nextFacing);
        if (nextFacing.equals(mFacing)) {
            return;
        }
